...
This is the working directory for the Claude Code agent embedded in the
aibrain UI over ACP (Agent Client Protocol) — the chat panel where the user
asks questions about their Obsidian vaults ("brains"). It is deliberately
separate from the `aibrain-kernel` dev checkout: this session should not pick
up the kernel repo's dev hooks, permissions, or skills, and the kernel repo's
dev session should not pick up vault-search instructions meant only for this
one.
...
Use the `vault-search` skill (`.claude/skills/vault-search/search.py`) before
...
This directory has its own `.claude/settings.json`, independent of the
aibrain-kernel dev config. If something here seems to need a dev-repo skill,
hook, or MCP server that isn't present, that's intentional — the two
environments are kept separate on purpose. Don't reach into
`../.claude/` for it.
